...
(** Gets the value from an option if it exists. Otherwise gives a default
    value. *)
...
(** Returns the second option only if the first is `Some _'. In a sense, this
    is the opposite of <|>. *)
...
(** * Removes all `None` options from the list. * * In fact, `cat_options` is
    implemented in terms of `filter_map`. *)
...
(** * Prints an option by doing nothing if it is `None`; else it uses the
    given printer. *)
val print
  :  (Format.formatter -> 'a -> unit)
  -> Format.formatter
