...
(** [find_index p l] is [(index, element)], the first element in [l]
    satisfying [p] along with its zero-based index.

    @raise Not_found if there is no element in [l] satisfying [p]. *)
val find_index : ('a -> bool) -> 'a list -> int * 'a

(** [find_index_opt p l] is the first element in [l] satisfying [p] along
    with its zero-based index in [l]. Returns [None] if there is no such
    element. *)
...
(** Same as {!partition}, but the predicate is applied to the zero-based
...
(** [partition_take k \[x_1; x_2; ...; x_n\]] is
    [(\[x_1; x_2; ...; x_k\], \[x_(k+1); x_(k+2); ...; x_n\])]. If [k < 0],
    then [(\[\], \[x_1; x_2; ...; x_n\])] is returned. If [k >= n], then
