...
let rec checkW cD cPsi sM sA =
  ...
  | (Root (loc, _, _, _), _ (* id *)), ((Atom (_, _, _), _) as sA) ->
...
and checkTuple loc cD cPsi (tuple, s1) (trec, s2) =
  let loop (tuple, s1) (typRec, s2) =
    ...
    | (Cons (tM, tuple), SigmaElem (_, tA, typRec)) ->
       checkW cD cPsi (tM, s1) (tA, s2);
       checkTuple loc cD cPsi (tuple, s1) (typRec, Dot (Obj tM, s2))
    | _ ->
       raise (Error (loc, TupleArity (cD, cPsi, (Tuple (loc, tuple), s1), (trec, s2)))) in
...
and inferHead loc cD cPsi head cl =
  ...
  | (FVar name, _) | (FMVar (name, _), _) | (FPVar (name, _), _) ->
...
and canAppear cD cPsi head sA loc =
  ...
  | CtxVar ctx_var ->
     ...
     begin
       try
         ignore (checkTypeAgainstSchemaProj loc cD cPsi head (TClo sA) name elems);
         true
       with
...
and checkSub loc cD cPsi1 ?(tM_opt = None) s1 cl cPsi1' =
  ...
  in
  let die () =
    IllTypedSub (cD, cPsi1, s1, cPsi1', tM_opt)
    |> throw loc
  in
  let rec checkSub loc cD cPsi s cPsi' =
    match (cPsi, s, cPsi') with
    | (cPsi, EmptySub, Null) -> ()
    | (cPsi, Undefs, Null) -> ()
    | (Null, Shift 0, Null) -> ()
    ...
    | (DDec (cPsi, _), Shift k, Null) ->
       if k > 0
       then checkSub loc cD cPsi (Shift (k - 1)) Null
       else die ()

    | (DDec (cPsi, _), Shift k, CtxVar psi) ->
       if k > 0
       then checkSub loc cD cPsi (Shift (k - 1)) (CtxVar psi)
       else die ()

    | (cPsi', Shift k, cPsi) ->
       if k >= 0
       then checkSub loc cD cPsi' (Dot (Head (BVar (k + 1)), Shift (k + 1))) cPsi
       else die ()
 ...
 *)
and checkTypRec cD cPsi (typRec, s) =
  match typRec with
  | SigmaLast (n, lastA) ->
...
and instanceOfSchElem cD cPsi (tA, s) (SchElem (some_part, block_part)) =
  ...
  let sArec =
    ...
    | (nonsigma, s') -> (SigmaLast (None, nonsigma), s')
  in
  dprintf
    begin
      fun p ->
...
and existsInstOfSchElemProj loc cD cPsi sA (h, i, n) elem =
  if i > n
  then raise (Error (loc, ParamVarInst (cD, cPsi, sA)))
  else
    begin
      try
...
and elemPostfix sArec sBrec =
  match (sArec, sBrec) with
  | ((SigmaLast _, _), (SigmaLast _, _)) ->
     None

  | ((SigmaElem (_, _, recA), s), (SigmaLast _, _)) ->
     Some (recA, s)

  | ((SigmaElem (_, _, recA), s), (SigmaElem (_, _, recB), s')) ->
...
and checkClObj cD loc cPsi' cM cTt =
  match (cM, cTt) with
  | (MObj tM, (MTyp tA, t)) ->
