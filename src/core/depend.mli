...
(** [max d1 d2] is {!explicit} if [d1] and [d2] are {!explicit}, and
    {!implicit} otherwise. *)
val max : t -> t -> t
...
(** [of_plicity p] is {!implicit} if [p] is {!Plicity.implicit}, and
    {!explicit} if [p] is {!Plicity.explicit}. *)
val of_plicity : Plicity.t -> t

(** [to_plicity d] is {!Plicity.implicit} if [d] is {!implicit}, and
    {!Plicity.explicit} if [d] is {!explicit}.

    @raise Error.Violation If [d] is {!inductive}. *)
val to_plicity : t -> Plicity.t

(** [to_plicity d] is {!Plicity.implicit} if [d] is {!implicit}, and
    {!Plicity.explicit} if [d] is {!explicit} or {!inductive}. This is a
    variant of {!to_plicity} that does not throw when [d] is {!inductive},
    and instead sends it to {!Plicity.explicit}. *)
val to_plicity' : t -> Plicity.t
