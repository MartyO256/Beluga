...
module Make (R : Store.Cid.RENDERER) : Printer.Int.T = struct
  ...
  let rec fmt_ppr_lf_typ cD cPsi lvl ppf =
    ...
    | LF.PiTyp ((LF.TypDecl (x, a), Depend.Implicit), b) ->
    ...
    | LF.PiTyp ((LF.TypDecl (x, a), Depend.Explicit), b) ->
       let x = fresh_name_dctx cPsi x in
       let cond = lvl > 0 in
  ...
  and fmt_ppr_lf_ctx_var cD ppf =
    ...
    | LF.CtxOffset psi ->
       ...
       | Some LF.(DeclOpt (u, Plicity.Implicit)) when !PC.printCtxUnderscore ->
          fprintf ppf "_"
       | Some LF.(Decl (u, _, Depend.Implicit)) when !PC.printCtxUnderscore ->
          fprintf ppf "_"
  ...
  and fmt_ppr_lf_kind cPsi lvl ppf =
    ...
    | LF.PiKind ((LF.TypDecl (x, a), Depend.Implicit), k) ->
       let x = fresh_name_dctx cPsi x in
       let cond = lvl > 0 in
    ...
    | LF.PiKind ((LF.TypDecl (x, a), Depend.Explicit), k) ->
       let x = fresh_name_dctx cPsi x in
       let cond = lvl > 0 in
  ...
  and isImplicitDecl =
    function
    | LF.Decl (_, _, Depend.Implicit) -> true
    | _ -> false
  ...
  let rec fmt_ppr_cmp_typ cD lvl ppf =
    ...
    | Comp.TypPiBox (_, LF.(Decl (u, CTyp w, Depend.Implicit) as d), tau) ->
       let cond = lvl > 1 in
       fprintf ppf "%s@[<2>(@[<2>%a@])@ @[%a@]%s@]"
         (l_paren_if cond)
         (fmt_ppr_lf_ctyp_decl cD) d
  ...
  let rec fmt_ppr_cmp_exp_chk cD cG lvl ppf =
    function
    | Comp.Syn (_, i) ->
  ...
  and fmt_ppr_cmp_exp_syn cD cG lvl ppf =
    function
    | Comp.Var (_, x) ->
       fprintf ppf "%s"
         (R.render_var cG x)
  ...
  let fmt_ppr_cmp_comp_prog_info ppf e =
    ...
    fprintf ppf
      ...
      (Nonempty.pp ~pp_sep: pp_print_cut fmt_ppr_cmp_total_dec) ds
