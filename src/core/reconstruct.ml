...
let elNumericOrder (tau : I.typ) (order : Ext.Comp.numeric_order)
  ...
  let rec skip tau n =
    ...
    | I.TypPiBox (_, Int.LF.Decl (u, cU, dep), tau), n ->
        dep
        |> Depend.fold
            ~implicit:(fun () -> 1 + skip tau n (* implicits are free *))
            ~explicit:(fun () -> 1 + skip tau (n - 1) (* explicit pi-types cost 1 *))
            ~inductive:(fun () ->
...
let rec elDCtxAgainstSchema loc recT cD psi s_cid =
  ...
  | Apx.LF.Null ->
     ...
     Int.LF.Null
  | Apx.LF.CtxHole ->
     Int.LF.CtxVar (Whnf.newCVar None cD (Some s_cid) Depend.implicit)
  ...
  | Apx.LF.CtxVar (Apx.LF.CtxName psi) ->
     ...
     begin
       ...
       with
       | Not_found ->
          FCVar.add psi (cD, Int.LF.Decl (psi, Int.LF.CTyp (Some s_cid), Depend.implicit));
...
let elClObj cD loc cPsi' clobj mtyp =
  ...
  | ( Apx.LF.Dot (Apx.LF.Obj (Apx.LF.Root (_, Apx.LF.Hole, Apx.LF.Nil)), Apx.LF.EmptySub)
    , Int.LF.PTyp _tA'
     ...
     let mV = Whnf.newMMVar' None (cD, Int.LF.ClTyp (mtyp, cPsi')) Depend.implicit in
...
let mgCompTypSpine cD (loc, cK) =
  let rec genMetaSpine =
    ...
    | (Int.Comp.PiKind (loc', Int.LF.Decl(u, cU, dep), cK), t) ->
       ...
       Int.Comp.MetaApp (mO, Whnf.cnormMTyp (cU, t), mS, Depend.to_plicity dep)
  in
...
let rec mgCtx cD' (cD, cPsi) =
  ...
  | Int.LF.CtxVar (Int.LF.CtxOffset psi_var) ->
     ...
     let v =
       let open Int.LF in
       Whnf.newMMVar' (Some n) (cD', CTyp (Some sW)) Depend.implicit
     in
...
let rec inferPatTyp' cD' (cD_s, tau_s) =
  ...
  | Int.Comp.TypPiBox (loc, (Int.LF.Decl (x, mtyp, dep)), tau) ->
     ...
     let tau' =
       inferPatTyp' (Int.LF.Dec (cD', Int.LF.Decl(x, mtyp', dep)))
         (Int.LF.Dec (cD_s, Int.LF.Decl(x, mtyp, dep)), tau)
     in
     Int.Comp.TypPiBox (loc, Int.LF.Decl (x, mtyp', dep), tau')
...
and elCommand cD cG =
  ...
  | A.Unbox (loc, i, x, modifier) ->
     ...
     in
     let d = Int.LF.(Decl (x, cU, Depend.explicit)) in
