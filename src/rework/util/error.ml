...
let register_printer f =
  Printexc.register_printer (fun e ->
      try Some (f e) with Match_failure _ -> None )
...
let register_printing_function
    (extract : exn -> 'a option) (fmt_ppr : Format.formatter -> 'a -> unit) :
    unit =
  let open F in
  register_printer'
...
let addInformation message = information := message :: !information

(** Register some basic printers. *)
let _ =
  register_printer (fun (Sys_error msg) ->
      print (fun ppf -> Format.fprintf ppf "System error: %s" msg) ) ;

  register_printer (fun (Violation msg) ->
      print (fun ppf ->
          Format.fprintf
            ppf
  ...
  register_printer (fun (NotImplemented (loc, msg)) ->
      print (fun ppf ->
