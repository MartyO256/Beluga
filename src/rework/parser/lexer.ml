  ...
  Error.register_printer' (function
      | Error (loc, e) ->
          ...
          Error.print_with_location loc (fun ppf ->
              ...
              | Violation msg ->
                  fprintf ppf "(internal error) %s" msg )
          |> Option.some
      | _ ->
          None )
...
let rec skip_nested_block_comment loc lexbuf =
  ...
  | _ ->
      throw
        !loc
        (Violation
           "catch-all case for skip_nested_block_comment should be \
            unreachable" )
...
let rec tokenize loc lexbuf =
  ...
  | block_comment_begin ->
      ...
      tokenize loc lexbuf
  | block_comment_end ->
      throw !loc MismatchedBlockComment
  | line_comment ->
      skip () ;
      tokenize loc lexbuf
  (* STRING LITERALS *)
  ...
  | "prop" ->
      const T.KW_PROP
  | "inductive" ->
      const T.KW_INDUCTIVE
  | "coinductive" ->
      const T.KW_COINDUCTIVE
  | "stratified" ->
      const T.KW_STRATIFIED
  | "LF" ->
      const T.KW_LF
  | "fun" ->
      const T.KW_FUN
  ...
  (* SYMBOLS *)
  | pragma ->
      T.PRAGMA (String.drop 2 (get_lexeme loc lexbuf))
  | arrow ->
      const T.ARROW
  | thick_arrow ->
      const T.THICK_ARROW
  ...
  | "+" ->
      const T.PLUS
  | hole ->
      T.HOLE (String.drop 1 (get_lexeme loc lexbuf))
  | "_" ->
      const T.UNDERSCORE
  | ident ->
      T.IDENT (get_lexeme loc lexbuf)
  | dot_number ->
  ...
  | hash_blank ->
      T.HASH_BLANK
  | hash_ident ->
      T.HASH_IDENT (get_lexeme loc lexbuf)
  | dollar_blank ->
      T.DOLLAR_BLANK
  | dollar_ident ->
      T.DOLLAR_IDENT (get_lexeme loc lexbuf)
  | "." ->
      const T.DOT
  | "#" ->
      const T.HASH
  ...
  | eof ->
      const T.EOI
  | number ->
      T.INTLIT (get_lexeme loc lexbuf |> int_of_string)
  | _ ->
      throw !loc (UnlexableCharacter (get_lexeme loc lexbuf))
