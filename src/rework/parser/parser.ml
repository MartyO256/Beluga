...
type state =
  ...
  ; backtrack : bool
  ; last_loc : Location.t
        (* Location of the last token seen by the parser. *)
  }
...
let print_path ppf (path : path) : unit =
  ...
  let print_entry ppf { label; location } : unit =
    fprintf
      ...
      "in `%s'%a"
      label
      (Option.print (fun ppf x -> fprintf ppf " at %a" Location.print x))
      location
  in
...
let _ =
  Error.register_printer' (function
      | Error (s, e) ->
          Some
            (Error.print_with_location e.location (fun ppf ->
                 print_error ppf e ) )
      | _ ->
          None )
...
let initial_state input =
  { input; backtrack = false; last_loc = Location.ghost }
 ...
 *)
let next_loc : Location.t parser = get_state $> fun s -> next_loc_at s

let prev_loc : Location.t parser = get_state $> fun s -> prev_loc_at s
 ...
 *)
let relabelling (type a) (p : a parser) (f : Location.t -> path -> path) :
    a parser =
  next_loc
  $ fun loc ->
  let open Either in
...
let peek : (Location.t * Token.t) option parser = anon_parser (fun s -> pure_at s (peek_at s))
                                 *)
