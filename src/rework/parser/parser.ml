(***** Beluga external syntax parser
  ...
  **** Intro to parser combinators
  ...
  where `state` contains a `(L.t * Token.t) LinkStream.t` for the
...
module Comp = External.Comp
module LF = External.LF
module Sgn = External.Signature
module T = Token
module F = Fun
...
type state =
  ...
  ; backtrack : bool
  ; last_loc : L.t
        (* L of the last token seen by the parser. *)
  }
...
(*
let entry (location : L.t) (label : string) : path' =
  Entry { location = Some location; label }
          *)

let rec path_head (p : path) : error_entry option =
  match p with [] -> None | x :: _ -> Some (path'_head x)
...
let print_content ppf : content -> unit =
  ...
  | `string_literal ->
      fprintf ppf "string literal"
  | `identifier i ->
      format_with ppf "identifier" string_option i
  | `qualified_identifier i ->
      fprintf
        ppf
        "qualified identifier%a"
        (format_option_with (fun ppf (ss, s) ->
             fprintf
               ppf
               "%a%s"
               (pp_print_list
  ...
  | `html_comment ->
      fprintf ppf "HTML comment"


type error' =
  (* External errors: the user's fault. *)
  | Unexpected of content (* expected *) * content (* actual *)
  | IllFormedDataDecl
  ...
  *)
   ...
   *)
  | Violation of string  (** Generic internal error. *)
...
and error =
  ...
  ; path : path
        (* ^ Sequence of parser labels that led to the parse error.
           ...
           ```
        *)
  ; location : L.t (* the location the error occurred at *)
  }
...
let print_path ppf (path : path) : unit =
  ...
  let print_entry ppf { label; location } : unit =
    fprintf
      ...
      "in `%s'%a"
      label
      (Option.print (fun ppf x -> fprintf ppf " at %a" L.pp x))
      location
  in
  let rec go' ppf (path' : path') =
    match path' with
    | Entry e ->
        fprintf ppf "%a" print_entry e
    | Shift (e, p) ->
        fprintf ppf "@[%a@]@,%a" print_entry e go_box p
  and go ppf (path : path) = pp_print_list go' ppf path
  and go_box ppf (path : path) = fprintf ppf "  @[<v>%a@]" go path in
...
let print_error ppf ({ path; location; _ } as e : error) =
  ...
  let (* rec *) g ppf { error = e; _ } =
    ...
    | Unexpected (t_exp, t_act) ->
        fprintf
          ppf
          "Unexpected token in stream@,  @[<v>Expected %a@,Got %a@]@,"
          print_content
          t_exp
          print_content
          t_act
    | IllFormedDataDecl ->
        fprintf
          ...
          ^^ "The type of a constructor must be a base type or function \
              type." )
    | WrongConstructorType (c, exp, act) ->
        fprintf
          ppf
          ( "Wrong datatype for constructor %a.@,  @[<v>"
          ^^ "Expected type %a@,"
          ^^ "Actual type %a"
          ^^ "@]" )
          Name.pp
...
let next_loc_at (s : state) : L.t =
  match LinkStream.observe s.input with
  | None ->
      failwith "lexer invariant failed"
  (* the lexer should infinitely repeat "EOI" when it's done. *)
  | Some ((loc, _), _) ->
      loc


let prev_loc_at (s : state) : L.t = s.last_loc

let initial_state input last_loc =
  { input; backtrack = false; last_loc }
...
let extract =
  let open Either in
  function s, Left e -> raise (Error (s, e)) | _, Right x -> x
 ...
 *)
let catch (p : 'a parser) (handler : state * 'a result -> state * 'b result)
    : 'b parser =
...
let fail_at' s location path error =
  (s, Either.Left { error; path; location })
...
let pure_at s x = (s, Either.Right x)

let pure x : 'a parser = { run = (fun s -> pure_at s x) }

(** Gets the current parser state. *)
let get_state : state parser = { run = (fun s -> pure_at s s) }

(** Sets the current parser state. *)
let put_state (s : state) : unit parser = { run = (fun _ -> pure_at s ()) }
...
let seq2 : type a b. a parser -> b parser -> (a * b) parser =
 fun p1 p2 -> p1 $ fun x -> p2 $ fun y -> pure (x, y)
...
let seq4 p1 p2 p3 p4 =
  p1
  $ fun x1 ->
  p2 $ fun x2 -> p3 $ fun x3 -> p4 $ fun x4 -> pure (x1, x2, x3, x4)
...
let seq5 p1 p2 p3 p4 p5 =
  ...
  p2
  $ fun x2 ->
  p3 $ fun x3 -> p4 $ fun x4 -> p5 $ fun x5 -> pure (x1, x2, x3, x4, x5)
 ...
 *)
let next_loc : L.t parser = get_state $> fun s -> next_loc_at s

let prev_loc : L.t parser = get_state $> fun s -> prev_loc_at s
 ...
 *)
let relabelling (type a) (p : a parser) (f : L.t -> path -> path) :
    a parser =
  next_loc
  $ fun loc ->
  let open Either in
      ...
      | s, Left e ->
          (s, Left { e with path = f loc e.path })
      | x ->
          x )
...
let shift p s =
  relabelling p (fun loc path ->
      [ Shift ({ location = Some loc; label = s }, path) ] )
...
let label p s =
  relabelling p (fun loc path ->
      Entry { location = Some loc; label = s } :: path )
...
let renamed label p =
  relabelling p (fun loc -> function
    | [] ->
        []
    | Shift (e, p) :: xs ->
        Shift ({ e with label }, p) :: xs
    | Entry e :: xs ->
        Entry { e with label } :: xs )
...
let rec traverse_ (f : 'a -> unit parser) (xs : 'a list) : unit parser =
  match xs with [] -> pure () | x :: xs -> f x &> traverse_ f xs
...
let peek : (L.t * Token.t) option parser = anon_parser (fun s -> pure_at s (peek_at s))
                                 *)
...
let alt (p1 : 'a parser) (p2 : 'a parser) : 'a parser =
  ...
  { run =
      (fun s ->
        ...
        | s', Left e ->
            let consumed_input =
              LinkStream.position s.input < LinkStream.position s'.input
            in
  ...
  { run =
      (fun s ->
        ...
        | None | Some ((_, T.EOI), _) ->
            pure_at s ()
        | Some ((_, t), _) ->
            fail_at s (Unexpected (`eoi, `token (Some t))) )
  }
...
let satisfy (f : T.t -> ('e, 'b) Either.t) : ('e, 'b) Either.t parser =
  { run =
      (fun s ->
        match LinkStream.observe s.input with
        | None ->
            failwith "lexer invariant failed: end-of-input is a token"
        | Some ((loc, t), xs) ->
            let r = f t in
            let s' =
...
let maybe (p : 'a parser) : 'a option parser =
  shifted "optionally" (alt (p $> Option.some) (pure None))
 ...
 *)
let many (p : 'a parser) : 'a list parser = shifted "many" (many' p)

(** `some p` repeats the parser `p` one or more times and collects the
    results in a list.
 *)
let some (p : 'a parser) : 'a list parser = shifted "some" (some' p)
...
let sep_by0 (p : 'a parser) (sep : unit parser) : 'a list parser =
  shifted
    ...
    { run =
        (fun s ->
          let q =
            seq (maybe p) (function
                | None ->
                    pure []
                | Some x ->
                    many' (sep &> p) $ fun xs -> pure (x :: xs) )
          in
          q.run s )
    }
...
let sep_by1 (p : 'a parser) (sep : unit parser) : 'a Nonempty.t parser =
  shifted
    ...
    { run =
        (fun s ->
          let q =
            seq2 p (many' (sep &> p)) $> fun (x, xs) -> Nonempty.from x xs
          in
          q.run s )
    }
...
let check_datatype_decl loc a cs : unit parser =
  ...
  traverse_
    (function
      ...
      | _ ->
          fail (Violation "check_datatype_decl invalid input") )
    cs
...
let check_codatatype_decl loc a cs : unit parser =
  ...
  traverse_
    (function
      ...
      | _ ->
          fail (Violation "check_codatatype_decl invalid input") )
    cs
...
let satisfy' (expected : content) (f : T.t -> 'a option) : 'a parser =
  satisfy (fun t ->
      f t |> Option.eliminate (Fun.const (Either.Left t)) Either.pure )
  |> span
  $ fun (loc, x) ->
  match x with
  | Either.Left t ->
      fail'
        [ Entry
            { location = Some loc
            ; label = Fmt.stringify print_content expected
            }
        ]
...
let tokens (ts : T.t list) : unit parser = traverse_ token ts
...
let identifier : string parser =
  satisfy' (`identifier None) (function T.IDENT s -> Some s | _ -> None)


let hash_identifier : string parser =
  satisfy' (`hash_identifier None) (function
      | T.HASH_IDENT s ->
          Some s
      | _ ->
          None )
...
let dollar_identifier : string parser =
  satisfy' (`dollar_identifier None) (function
      | T.DOLLAR_IDENT s ->
          Some s
      | _ ->
          None )
...
let hash_blank : unit parser =
  satisfy' `hash_blank (function T.HASH_BLANK -> Some () | _ -> None)


let dollar_blank : unit parser =
  satisfy' `dollar_blank (function T.DOLLAR_BLANK -> Some () | _ -> None)


let namify (p : string t) : Name.t t =
  p |> span $> fun (location, value) -> Name.make location value
...
let dep_name_of_nb = function
  | `blank location ->
      (Depend.implicit, Name.make_blank location)
  | `name name ->
      (Depend.explicit, name)


let dot_name : Name.t t = token T.DOT &> name

let hash_name : Name.t t = namify hash_identifier

let hash_name_or_blank : name_or_blank t =
  alt (hash_name $> fun x -> `name x) (hash_blank |> blankify)


let dollar_name : Name.t t = namify dollar_identifier

let dollar_name_or_blank : name_or_blank t =
  alt (dollar_name $> fun x -> `name x) (dollar_blank |> blankify)
...
let name_or_blank' : name_or_blank name_parser = function
  | `ordinary ->
      name_or_blank
  | `dollar ->
      dollar_name_or_blank
  | `hash ->
      hash_name_or_blank


let name' : Name.t name_parser = function
  | `ordinary ->
      name
  | `dollar ->
      dollar_name
...
let integer : int parser =
  satisfy' (`integer None) (function T.INTLIT k -> Some k | _ -> None)


let dot_integer : int parser =
  satisfy' `dot_integer (function T.DOT_NUMBER k -> Some k | _ -> None)
...
let fqname =
  ...
  |> span
  $> fun (location, path) ->
  let modules, name = Nonempty.unsnoc path in
  FullyQualifiedName.make location modules name


let pragma s = token (T.PRAGMA s)

let string_literal =
  satisfy' `string_literal (function T.STRING s -> Some s | _ -> None)
...
let bracks_or_opt_parens' prefix p =
  alt (prefix &> alt (bracks p) (parens p)) p


(** See bracks_or_opt_parens'. This is a prefixless instance. *)
let bracks_or_opt_parens p = bracks_or_opt_parens' (pure ()) p
...
let sgn_name_pragma : Sgn.decl parser =
  seq3 (pragma "name" &> name) identifier (maybe identifier <& token T.DOT)
...
let rec lf_kind =
  { run =
      (fun s ->
        let pi_kind =
          seq2 (lf_typ_decl () |> braces <& maybe (token T.ARROW)) lf_kind
          |> span
          $> fun (loc, (d, k)) -> LF.PiKind (loc, d, k)
        in
        let arr_kind =
          seq2 (lf_typ_atomic <& token T.ARROW) lf_kind
          |> span
  ...
  { run =
      (fun s ->
        let lam =
          ...
          $> fun (loc, (_, x, _, ms)) -> LF.Lam (loc, x, ms)
        in
        let atomic =
          choice
            ...
            ; ( span (parens (seq2 lf_typ (maybe (token T.COLON &> lf_typ))))
              $ fun (loc, (m, q)) ->
              match (q, m) with
              | None, LF.AtomTerm (_, t) ->
                  pure t
              | None, _ ->
                  LF.NTyp (loc, m) |> pure
              | Some a, LF.AtomTerm (_, t) ->
                  LF.Ann (loc, t, a) |> pure
              | _, _ ->
                  fail (Violation "invalid atomic LF term") )
        ...
        in
        (choice [ lam; head; atomic ] |> labelled "LF term").run s )
  }
...
and lf_typ_decl () =
  seq2 (name <& token T.COLON) lf_typ
  ...
  { run =
      (fun s ->
        let pi =
          ...
          match k_or_a with
          | `Kind k ->
              `Kind (LF.PiKind (loc, name, k))
          | `Typ a ->
              `Typ (LF.PiTyp (loc, name, a))
        in
        let arrow =
          seq2 lf_typ_atomic (maybe (token T.ARROW &> lf_kind_or_typ))
          |> span
          ...
          | None ->
              `Typ a
          | Some (`Kind k) ->
              `Kind (LF.ArrKind (loc, a, k))
          | Some (`Typ a') ->
              `Typ (LF.ArrTyp (loc, a, a'))
        in
        let p =
          choice [ pi; (type_kind $> fun a -> `Kind a); arrow ]
      ...
      (fun s ->
        let p =
          ...
          choice
            ...
            ; ( seq2 (trying (lf_typ_atomic <& token T.ARROW)) lf_typ
              |> span
...
let sgn_lf_typ_decl : Sgn.decl parser =
  let lf_typ_decl_body (* cmp_dat *) =
    let typ_decl = seq2 (name <& token T.COLON) lf_kind in
    seq2
      (typ_decl <& token T.EQUALS)
      (maybe (token T.PIPE) &> sep_by0 lf_const_decl (token T.PIPE))
    |> span
...
let hole : string option parser =
  satisfy' (`hole None) (function
      | T.HOLE s ->
          Some (match s with "" -> None | _ -> Some s)
      | _ ->
          None )
  |> labelled "hole"
...
let clf_projection : LF.proj parser =
  alt (dot_integer $> fun k -> LF.ByPos k) (dot_name $> fun x -> LF.ByName x)
...
let rec lf_function_type p p_atomic =
  let pi =
    ...
    in
    seq2 d p |> span $> fun (loc, (d, b)) -> LF.PiTyp (loc, d, b)
  in
  let arrow =
...
and clf_term_app =
  { run =
      (fun s ->
        let normal_list =
          some clf_normal |> span $> fun (loc, ms) -> LF.TList (loc, ms)
        in
        let p =
          choice
            [ normal_list
            ...
            ; (span clf_typ $> fun (loc, a) -> LF.NTyp (loc, a))
            ]
        ...
        let a =
          labelled
            ...
            | [ LF.NTyp (_, a) ] ->
                a
            | _ ->
                LF.AtomTerm (loc, LF.TList (loc, ms)) )
        in
        let b =
          seq2 fqname (many clf_normal)
          |> span
          $> fun (loc, (x, ms)) ->
          LF.AtomTerm
            ( loc
            , LF.TList
                (loc, LF.Root (loc, LF.Name (loc, x, None), LF.Nil) :: ms) )
        in
        let p =
          choice
            [ a
...
and clf_typ_rec_block =
  { run =
      (fun s ->
        let p =
          rec_block clf_typ_rec_elem |> labelled "contextual LF block"
        in
        p.run s )
  }
...
and clf_typ =
  { run =
      (fun s ->
        let pi_decl = seq2 (name <& token T.COLON) clf_typ |> braces in
        let pi =
          seq2 (pi_decl <& maybe (token T.ARROW)) clf_typ
          |> span
          $> fun (loc, ((x, a), a')) -> LF.PiTyp (loc, LF.TypDecl (x, a), a')
        in
        let arrow_or_atomic =
          seq2 clf_typ_atomic (maybe (token T.ARROW &> clf_typ))
          |> span
          $> fun (loc, (a, b)) ->
          match b with None -> a | Some b -> LF.ArrTyp (loc, a, b)
        in
        (choice [ pi; arrow_or_atomic ] |> labelled "contextual LF type").run
          s )
  }
      ...
      (fun s ->
        let p =
          choice
            ...
            ; ( seq2 dollar_name (maybe (bracks clf_sub_new))
              |> span
...
and clf_sub_new =
  { run =
      (fun s ->
        let start =
          alt
            (clf_sub_term $> fun t -> (t, []))
            (span clf_term_app $> fun (loc, tM) -> (LF.EmptySub loc, [ tM ]))
        in
        let nonemptysub =
        ...
        in
        let p =
          alt nonemptysub emptysub |> labelled "contextual LF substitution"
        in
        p.run s )
  }
  ...
  { run =
      (fun s ->
        let var =
          seq3
            (alt
               (hash_name $> fun x loc sigma -> LF.PVar (loc, x, sigma))
               (fqname $> fun x loc sigma -> LF.Name (loc, x, sigma)) )
          ...
          match proj with Some k -> LF.Proj (loc, m, k) | None -> m
        in
        let hole =
          token T.UNDERSCORE |> span $> fun (loc, _) -> LF.Hole loc
        in
...
and clf_normal =
  { run =
      (fun s ->
        let lam =
          seq2 (token T.LAMBDA &> name) (token T.DOT &> clf_term_app)
          |> span
          |> labelled "LF lambda"
          $> fun (loc, (x, m)) -> LF.Lam (loc, x, m)
        in
        (*
      let modul =
       ...
       *)
        let head =
          span clf_head $> fun (loc, h) -> LF.Root (loc, h, LF.Nil)
        in
        let app =
          seq2 clf_term_app (maybe (token T.COLON &> clf_typ))
          |> parens
          |> span
          |> labelled "LF application"
          $> fun (loc, (tm, ann)) ->
          match ann with None -> tm | Some a -> LF.Ann (loc, tm, a)
        in
        let lfhole =
          span hole
          |> labelled "LF hole"
...
and clf_dctx : LF.dctx parser =
  { run =
      (fun s ->
        let clf_typ_decl =
          seq2 name (maybe (token T.COLON &> clf_typ))
          $> fun (x, tA) ->
          match tA with
          | Some tA ->
              LF.TypDecl (x, tA)
        ...
        *)
        ...
        let p =
          choice
            [ ( seq2 start (many (token T.COMMA &> clf_typ_decl))
              $> fun (cPsi, ds) ->
...
and contextual : type a. a parser -> (LF.dctx * a) parser =
 fun p -> seq2 (clf_dctx <& token T.TURNSTILE) p
...
let meta_obj =
  { run =
      (fun s ->
        let p =
          let clobj =
            seq2 clf_dctx (maybe (token T.TURNSTILE &> clf_sub_new))
            |> span
            $> fun (loc, (cPsi, tR)) ->
            match tR with
            | Some tR ->
                (loc, LF.ClObj (cPsi, tR))
            | None ->
                (loc, LF.CObj cPsi)
          in
          clobj |> bracks |> labelled "meta object"
        in
        p.run s )
  }
...
let ctype_kind = token T.KW_CTYPE |> span $> fun (loc, _) -> Comp.Ctype loc
...
let contextual_variable_decl
    ...
    ('name * 'b) t =
  seq2 (name <& token T.COLON) (box (contextual p))
...
let cltyp : (LF.dctx * typ_or_ctx) parser =
  labelled
    "boxed type"
    (let typ =
       contextual clf_typ_atomic |> bracks $> fun (cPsi, a) -> (cPsi, `Typ a)
     in
     let ctx =
       contextual clf_dctx |> bracks $> fun (cPsi, cPhi) -> (cPsi, `Ctx cPhi)
     in
     alt
...
let clf_ctyp_decl_bare :
    type a. a name_parser -> (a -> Depend.t * Name.t) -> LF.ctyp_decl t =
      ...
      (fun s ->
        ...
        let dollar_variable_decl p =
          contextual_variable_decl
            ...
            p
        in
        let mk_decl dep f (loc, (p, w)) = LF.Decl (p, (loc, f w), dep) in
        ...
        let subst_variable =
          ...
          |> span
          $> mk_cltyp_decl_blank (fun (sclass, cPhi) ->
                 LF.STyp (sclass, cPhi) )
...
let ctx_variable =
  labelled
    ...
    ( seq2
        ...
        (fqname <& not_followed_by meta_obj)
    |> span
  ...
  { run =
      (fun s ->
        ...
        let subst_variable =
          ...
          in
          labelled
            "substitution/renaming variable"
            ( dollar_variable_decl (seq2 subst_class clf_dctx)
            |> span
...
let mctx ?(sep = token T.COMMA) p =
  sep_by0 p sep $> List.fold_left (fun acc x -> LF.Dec (acc, x)) LF.Empty


let pibox p r f =
  seq2 (p <& maybe (token T.ARROW)) r
  |> span
  $> fun (loc, (ctyp_decl, x)) -> f loc ctyp_decl x


let arrow atomic r f =
  seq2 (trying (atomic <& token T.ARROW)) r
  |> span
...
let rec cmp_typ =
  { run =
      (fun s ->
        let ctx_pibox =
          labelled
            "Context variable Pi-box type"
            (pibox (ctx_variable |> parens) cmp_typ (fun loc decl tau ->
        ...
        let pibox =
          labelled
            "Pi-box type"
            (pibox
               ( clf_ctyp_decl_bare name' (fun x -> (Depend.explicit, x))
               |> braces )
               cmp_typ
               (fun loc ctyp_decl tau -> Comp.TypPiBox (loc, ctyp_decl, tau)) )
        in
        let arr =
          labelled
            "Arrow computation type"
            (arrow cmp_typ_cross cmp_typ (fun loc a1 a2 ->
                 Comp.TypArr (loc, a1, a2) ) )
        in
        let p =
          choice [ pibox; ctx_pibox; arr; cmp_typ_cross ]
...
and cmp_typ_cross =
  { run =
      (fun s ->
        let p =
          seq2 cmp_typ_atomic (maybe (token T.STAR &> cmp_typ_cross))
          |> span
      ...
      (fun s ->
        ...
        let pbox =
          ...
          Comp.TypBox
            ...
            , ( loc
              , LF.ClTyp
                  (LF.PTyp (LF.AtomTerm (loc, LF.TList (loc, ms))), cPsi) )
            )
        in
        let sub =
          token T.DOLLAR
          ...
          |> span
          $> fun (loc, (cPsi, cPhi)) ->
          Comp.TypBox (loc, (loc, LF.ClTyp (LF.STyp (LF.Subst, cPhi), cPsi)))
        in
        let ctx =
          span fqname
          $> fun (loc, schema) -> Comp.TypBox (loc, (loc, LF.CTyp schema))
        in
        let ordinary =
          seq2 (trying (clf_dctx <& token T.TURNSTILE)) (some clf_normal)
          |> span
          |> labelled "boxed type"
          $> fun (loc, (cPsi, ms)) ->
          Comp.TypBox
            ...
            , ( loc
              , LF.ClTyp
                  (LF.MTyp (LF.AtomTerm (loc, LF.TList (loc, ms))), cPsi) )
            )
        in
        let p =
          choice
  ...
  { run =
      (fun s ->
        let pibox =
          labelled
            "Pi-box kind"
            ( seq2 (trying (clf_ctyp_decl <& maybe (token T.ARROW))) cmp_kind
            |> span
            $> fun (loc, (ctyp_decl, k)) ->
        ...
        let arrow =
          labelled
            "arrow kind"
            ( seq2 (trying (cltyp <& token T.ARROW) |> span) cmp_kind
            |> span
        ...
        in
        let p =
          choice [ ctype_kind; pibox; arrow ] |> labelled "computation kind"
        in
        p.run s )
  }
...
let nested
    ...
    (p : a parser)
    (g : a located -> a)
    (f : a located -> a located -> a) : a parser =
  sep_by1 (span p) (token T.COMMA)
  |> span
  |> parens
  $> fun (loc', l) -> Nonempty.fold_left g (fun acc p -> f (loc', acc) p) l
...
(* We use a left fold because the tuples in Beluga naturally nest on the left.
   ...
   should join the locations of its inputs. L joining uses
 ...
 *)
let case_pragma =
  maybe_default (pragma "not" &> pure Comp.PragmaNotCase) Comp.PragmaCase
...
and cmp_pattern =
  { run =
      (fun s ->
        ...
        let p =
          seq2 pattern (maybe (token T.COLON &> cmp_typ))
          |> labelled "possibly annotated pattern"
          |> span
          $> fun (loc, (p, tau)) ->
          match tau with None -> p | Some tau -> Comp.PatAnn (loc, p, tau)
        in
        p.run s )
  }
      ...
      (fun s ->
        ...
        let nested =
          nested cmp_pattern Pair.snd (fun (l1, p1) (l2, p2) ->
              Comp.PatPair (L.join l1 l2, p1, p2) )
        ...
        in
        let p =
          choice [ mobj_pat; nested; var ] |> labelled "bare pattern"
        in
        p.run s )
  }
  ...
  { run =
      (fun s ->
        let abstraction param c =
          ...
          |> span
          $> fun (loc, (params, i)) ->
          List.fold_left (fun acc f -> c loc f acc) i (List.rev params)
        in
        let fn =
          token T.KW_FN
          &> abstraction name (fun loc f acc -> Comp.Fn (loc, f, acc))
          |> labelled "ordinary function abstraction"
        in
        let mlam =
          token T.KW_MLAM
          &> abstraction
               (choice [ hash_name; dollar_name; name ])
        ...
        let case =
          ...
          $> fun (loc, (i, prag, bs)) -> Comp.Case (loc, prag, i, bs)
        in
        let impossible =
        ...
        let nested =
          nested cmp_exp_chk Pair.snd (fun (l1, e1) (l2, e2) ->
              Comp.Pair (L.join l1 l2, e1, e2) )
        in
        let hole = hole |> span $> fun (loc, h) -> Comp.Hole (loc, h) in
        let box_hole =
          token T.UNDERSCORE |> span $> fun (loc, _) -> Comp.BoxHole loc
        in
        let meta_obj =
          meta_obj |> span $> fun (loc, tR) -> Comp.Box (loc, tR)
        in
        let p =
          choice
...
and cmp_copat_spine =
  ...
  { run =
      (fun s ->
        let p =
          seq2 (go <& token T.THICK_ARROW) cmp_exp_chk
...
and cmp_exp_syn =
  { run =
      (fun s ->
        let p =
          seq2 cmp_exp_syn' (many (span cmp_exp_chk''))
          |> span
          $> fun (loc, (i, es)) ->
          let rec fold i es =
            match es with
            | [] ->
                i
            | (loc', e) :: es ->
                let i = Comp.Apply (L.join loc loc', i, e) in
                fold i es
          in
          fold i es
        in
      ...
      (fun s ->
        ...
        let nested =
          nested cmp_exp_syn Pair.snd (fun (l1, i1) (l2, i2) ->
              Comp.PairVal (L.join l1 l2, i1, i2) )
        ...
        in
        let p = choice [ name; meta_obj; nested ] in
        p.run s )
  }


let call_arg =
  alt (name $> Option.some) (token T.UNDERSCORE &> pure None)
...
let total_decl : Comp.total_dec t =
  let total =
    ...
    &> alt
         ...
         |> span
         $> fun (loc, order) -> Comp.NumericTotal (loc, order) )
  in
  alt trust_order total |> labelled "totality declaration"
...
let gctx =
  ...
  $> List.fold_left (fun acc x -> LF.Dec (acc, x)) LF.Empty


(** Mutual block of computation type declarations. *)
let sgn_cmp_typ_decl =
  labelled
    "Inductive or stratified computation type declaration"
    (let cmp_typ_decl =
       let flavour =
         alt
     ...
     let cmp_cotyp_decl =
       let cmp_cotyp_body =
         seq2
           ...
           *)
           ...
           ( opt_parens (seq2 (name <& token T.COLON) cmp_typ)
       ...
       $> fun () -> (Sgn.CompCotyp { location; identifier; kind }, decls)
     in
...
let sgn_query_pragma =
  let bound =
    alt (token T.STAR &> pure None) (integer $> Option.some)
    |> labelled "search bound"
  in
  pragma "query"
  &> seq3 (seq2 bound bound) (maybe (name <& token T.COLON)) lf_typ
  <& token T.DOT
  |> span
...
let sgn_oldstyle_lf_decl =
  labelled
    ...
    ( seq2 (name <& token T.COLON) (lf_kind_or_typ <& token T.DOT)
    |> span
    $> fun (location, (identifier, k_or_a)) ->
    match k_or_a with
    | `Kind kind ->
        Sgn.Typ { location; identifier; kind }
    | `Typ typ ->
...
let associativity =
  [ ("left", Sgn.Left); ("right", Sgn.Right); ("none", Sgn.None) ]
  |> List.map (fun (k, r) ->
         keyword k &> pure r |> labelled ("associativity `" ^ k ^ "'") )
  |> choice
...
let sgn_fixity_pragma : Sgn.decl parser =
  let infix_pragma : Sgn.decl parser =
    ...
    Sgn.Pragma
      { location; pragma = Sgn.FixPrag (x, Sgn.Infix, precedence, assoc) }
  in
...
let sgn_comment : Sgn.decl parser =
  satisfy' `html_comment (function T.BLOCK_COMMENT s -> Some s | _ -> None)
  |> span
...
let lf_schema_some : LF.typ_decl LF.ctx parser =
  alt
    ( token T.KW_SOME
    &> bracks (sep_by0 (lf_typ_decl ()) (token T.COMMA))
...
let lf_typ_rec_block = rec_block lf_typ_rec_elem |> labelled "LF block"
...
let lf_typ_rec =
  alt
    ...
    $> fun a ->
    let x = match a with LF.Atom (_, n, _) -> Some n | _ -> None in
    LF.SigmaLast (x, a) )
...
let lf_schema_elem =
  ...
  |> span
  $> fun (loc, (s, a)) -> LF.SchElem (loc, s, a)


let sgn_schema_decl : Sgn.decl parser =
  seq2
    (token T.KW_SCHEMA &> name <& token T.EQUALS)
    (sep_by1 lf_schema_elem (token T.PLUS) $> Nonempty.to_list)
  <& token T.SEMICOLON
  |> span
...
let sgn_let_decl : Sgn.decl parser =
  seq2
    (token T.KW_LET &> seq2 name (maybe (token T.COLON &> cmp_typ)))
...
let harpoon_command : Comp.command parser =
  let by =
    token T.KW_BY
    &> seq3 (cmp_exp_syn <& token T.KW_AS) name (maybe_default boxity `boxed)
    |> span
    ...
    | `boxed ->
        Comp.By (loc, i, x)
    | `unboxed ->
        Comp.Unbox (loc, i, x, None)
    | `strengthened ->
        Comp.Unbox (loc, i, x, Some `strengthened)
  in
  let unbox =
    keyword "unbox"
    &> seq2 (span cmp_exp_syn <& token T.KW_AS) name
...
let case_label : Comp.case_label parser =
  let extension_case_label =
    trying (keyword "extended" &> token T.KW_BY)
    &> integer
    |> span
  ...
  let pvar_case_label =
    token T.HASH
    &> seq2 (maybe_default integer 1) (maybe dot_integer)
    |> span
...
let rec harpoon_proof : Comp.proof parser =
  { run =
      (fun s ->
        ...
        in
        let command_proof =
          seq2 (harpoon_command <& token T.SEMICOLON) harpoon_proof
          |> span
          $> fun (loc, (cmd, prf)) -> Comp.Command (loc, cmd, prf)
        in
        let directive_proof =
          harpoon_directive |> span $> fun (loc, d) -> Comp.Directive (loc, d)
        in
        let p =
        ...
        let p =
          choice
            ...
            ; (let suffices_arg =
               ...
               &> seq2 (cmp_exp_syn <& token T.KW_TOSHOW) (many suffices_arg)
               |> span
...
and harpoon_hypothetical : Comp.hypothetical parser =
  ...
  let hypotheses =
    seq2
      ( mctx (clf_ctyp_decl_bare name_or_blank' dep_name_of_nb)
      <& token T.PIPE )
      gctx
    $> fun (cD, cG) -> { cD; cG }
  in
  { run =
      (fun s ->
        let p =
          seq2 (hypotheses <& token T.SEMICOLON) harpoon_proof
          |> braces
          |> span
...
and harpoon_split_branch : Comp.split_branch parser =
  { run =
      (fun s ->
        let p =
          token T.KW_CASE
          &> seq2 (case_label <& token T.COLON) harpoon_hypothetical
          |> span
...
let proof_decl : Sgn.thm_decl parser =
  token T.KW_PROOF &> thm (harpoon_proof $> fun p -> Comp.Proof p)


let program_decl : Sgn.thm_decl parser =
  token T.KW_REC &> thm (cmp_exp_chk $> fun e -> Comp.Program e)


let sgn_thm_decl : Sgn.decl parser =
  sep_by1 (choice [ program_decl; proof_decl ]) (token T.KW_AND)
  <& token T.SEMICOLON
  |> span
...
let interactive_harpoon_command =
  let module H = External.Harpoon in
  let intros =
    keyword "intros" &> maybe (some identifier) $> fun xs -> H.Intros xs
  in
  let split =
    keyword "split" &> cmp_exp_syn $> fun t -> H.(Split (`split, t))
  in
  let msplit =
    keyword "msplit"
  ...
  in
  let invert =
    keyword "invert" &> cmp_exp_syn $> fun t -> H.(Split (`invert, t))
  in
  let impossible =
    token T.KW_IMPOSSIBLE
    &> cmp_exp_syn
  ...
  in
  let solve = keyword "solve" &> cmp_exp_chk $> fun t -> H.Solve t in
  let by =
    token T.KW_BY
    &> seq3 cmp_exp_syn (token T.KW_AS &> name) (maybe_default boxity `boxed)
    $> fun (i, name, b) ->
    match b with
    | `strengthened ->
        H.Unbox (i, name, Some `strengthened)
    | `unboxed ->
        H.Unbox (i, name, None)
    | `boxed ->
        H.By (i, name)
  in
  let compute_type = token T.KW_TYPE &> cmp_exp_syn $> fun i -> H.Type i in
  let suffices =
    let tau_list_item =
      alt
    ...
    in
    seq2
      (tokens [ T.KW_SUFFICES; T.KW_BY ] &> cmp_exp_syn)
      (token T.KW_TOSHOW &> sep_by0 tau_list_item (token T.COMMA))
    $> fun (i, tau_list) -> H.Suffices (i, tau_list)
  in
  let unbox =
    keyword "unbox"
    &> seq2 cmp_exp_syn (token T.KW_AS &> name)
  ...
  let rename =
    let level =
      choice [ keyword "comp" &> pure `comp; keyword "meta" &> pure `meta ]
    in
    keyword "rename"
  ...
  in
  let basic_command =
    choice [ keyword "list" &> pure `list; keyword "defer" &> pure `defer ]
  in
  let select_theorem =
    keyword "select" &> name $> fun x -> H.SelectTheorem x
  in
  let create_command, serialize_command =
    (keyword "create" &> pure `create, keyword "serialize" &> pure `serialize)
  in
  let theorem_command =
    let dump_proof =
    ...
    &> choice
         ...
         :: dump_proof
         :: List.map
              (fun (k, k') -> keyword k &> pure k')
              [ ("show-ihs", `show_ihs); ("show-proof", `show_proof) ] )
  ...
  in
  let subgoal_command =
    keyword "subgoal" &> basic_command $> fun cmd -> H.Subgoal cmd
  in
  let defer = keyword "defer" &> pure (H.Subgoal `defer) in
  let info_kind = choice [ keyword "theorem" &> pure `prog ] in
  let info =
    keyword "info" &> seq2 info_kind name $> fun (k, name) -> H.Info (k, name)
  in
  let translate =
    keyword "translate" &> name $> fun name -> H.Translate name
  in
